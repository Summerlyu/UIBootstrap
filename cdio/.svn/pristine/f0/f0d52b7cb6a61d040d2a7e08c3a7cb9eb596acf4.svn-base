...
Calendar.prototype = {
	...
	_init: function(config) {
		 ...
		 * @type String
		 */		
		this.sMessage = config.sMessage || "<span><strong>时间与日期</strong></span>";
		/**
		 * 存放出发日期日历实例
		...
		_CAL.on(obj, "click", function(e) {
			...
			switch(oTarget.className) {
				case "cal-close":
					...
					break;
				case "cal-prev":
					lzc.id(that.id).className == 'time-end' ? that.prevMonthY() : that.prevMonth();
					...
					break;
				case "cal-next":
					lzc.id(that.id).className == 'time-end' ? that.nextMonthY() : that.nextMonth();
					that.y = oYear.toString().substring(2);
					that.endDate = oYear+'-'+lzc.zero(timeMonth)+'-'+that.r;
					that.onSelectDate(oTarget);
					that.empty(that);
			...
			if(oTarget.tagName.toUpperCase() === "A" && oTarget.className != "disabled") {
				...
				if(that.triggerNode.tagName.toUpperCase() === "INPUT") {					
					if(lzc.id(that.id).className == 'time-end')
					{
						lzc.Class(lzc.id(that.id),'check')[0].checked=false;
						var oEvent = e || event;
			...
			if(lzc.Class(lzc.id(that.id),'check')[0])
			{
				if(oTarget.tagName.toUpperCase() === 'INPUT')
				{
					...
					{
						that.empty(that);
						that.triggerNode.value = that.y+'-'+lzc.zero(that.m)+'-'+that.r+' 00:00-23:59';
						that.s = that.f = that.f1 = null;
						that.hide();
					}
				}
