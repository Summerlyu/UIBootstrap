...
<body data-target=".bs-docs-sidebar" data-spy="scro"
	...
	<div>
		<s:form id="searchCon" method="post"
			action="loadTeacherJudgeHistoryPage_adminQueryJudge">
			<div class="control-group">
				...
				<s:select class="span2" name="studentName"
					list="assessmentList" headerKey="" headerValue="==请选择==">
				</s:select>

				<span><font size="4">老师:</font></span>
				<s:select class="span2" name="teacherName" list="appraiserList"
					headerKey="" headerValue="==请选择==">
				</s:select>
			...
			<s:iterator value="teacherPage.results" status="status">
				<tr>
					<td width="10%"><s:if test="judgeMerit==\"5\"">优秀</s:if> <s:elseif
							...
							test="judgeMerit==\"1\"">不及格</s:elseif> <s:else>无效类型</s:else>
					</td>

					<td width="40%"><s:property value="judgeContent" /> <br />[
						<s:property value="judgeTime" /> ]</td>
					<td width="15%">学生:<s:property value="studentName" />
					</td>
					<td width="15%">老师:<s:property value="teacherName" />
					</td>
	...
	<div class="pagination pagination-right">
		<ul>
			...
			<s:else>
				...
				<s:if test="index!=1">
					<li><a href="#"
						onclick="loadPage(<s:property value="index-1"/>);"><<</a>
					</li>
				</s:if>
				<%-- 当前页后面剩余页数小于5页--%>
